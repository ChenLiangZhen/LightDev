...
const Card: React.FC<Card> = ({selected, title, description}) => {
   ...
   return <motion.button onClick={()=> console.log("clicked")}
         ...
         <Text className={"tracking-wider dark:text-neutral-300 dark:group-hover:text-neutral-50 text-neutral-600 group-hover:text-neutral-800 transition-colors duration-300 mb-1.5"} fontSize={FontSize.lg} bold>{title}</Text>
         <Text className={"dark:text-neutral-300 dark:group-hover:text-neutral-50 text-neutral-600 group-hover:text-neutral-800 transition-colors duration-300"} fontSize={FontSize.sm} light>{description}</Text>
